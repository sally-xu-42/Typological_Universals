...
# ==== TATSUKI ====
ja_copula_adverb = ["である", "でない", "ではない", "じゃない", "らしい", "かもしれない"]
ja_copula_verb = ["居る", "有る", "成る"]
...
def reverse_content_head(sentence,
                         ...
                         SPECIAL_CC=False, SPECIAL_COP=False, SPECIAL_ACL=False):
    ...
    """
    if SPECIAL_COP and language != "ja":
            ...
            if sentence[i]["dep"] == "cop" or sentence[i]["dep"].startswith("cop:"):
                ...
                for j in range(len(sentence)):
                    if sentence[j]["head"] == head + 1 and j != i and sentence[j]["dep"] in ["nsubj", "csubj", "cc", "conj", "mark", "aux", "advmod"]:
                        if sentence[j]["dep"] == "conj" and sentence[j]["posUni"] not in VERB_POS:
                            continue
                        sentence[j]["head"] = i + 1
    ...
    if SPECIAL_CC: # SPECIAL_CC take care when we need to differentiate cc connecting verbs or non-verbs
        ...
        for i in range(len(sentence)): # deal with conj without cc
            if sentence[i]["dep"] == "conj" and children_check(i, "lifted_cc", sentence):
                ...
                # 这里不用查有没有conj,为了语法对称可以留下。conj在没有and的情况下不可能被标记
...
class CorpusIteratorFuncHead:
    def __init__(
        ...
        self.SPECIAL_ACL = False
        # self.SPECIAL_ACL = True if language == "ja" else False
